...
options(digits=2)
# setwd("/home/miguel/Documentos/ecomicR/markdown_ex/")
...
<<label=sptab, echo=FALSE, include=TRUE, xresults=tex>>=
...
<<label=hills,echo=FALSE, include=FALSE>>=
...
Dos $\Sexpr{ncol(fito)}$ feitos entre o 1951 e 1952 na ría de Vigo, Margalef e compañeiros observaron entre $\Sexpr{min(div51)}$ e $\Sexpr{max(div51)}$ especies de fitoplancto despois de recontar unha media de $\Sexpr{sum(fito)}$ de \href{https://gl.wikipedia.org/wiki/Diatomeas}{diatomeas} e \href{https://gl.wikipedia.org/wiki/Dinoflaxelados}{dinoflaxelados}.
...
<<label=hills_summary, echo=FALSE>>=
...
<<label=sac, echo=FALSE, fig=TRUE, width=6, height=4>>=
...
<<label=wittaker, echo=FALSE, fig=TRUE, height=4, width=6>>=
