...
<<label=global_options, include=FALSE, echo=FALSE>>=
options(digits=2)

# Installa and load packages
...
\begin{figure}[htp]
    \centering
    \includegraphics[width=0.8\textwidth]{./imaxes/logomargalef.png}
...
<<label=abun, echo=FALSE, include=FALSE>>=
# Define o cartafol de datos que usarás
tabII <- read.csv("data/tabII.csv", sep=";")
tabII[is.na(tabII)] <- 0
# Comproba a importación
# str(tabII)
...
% Define a cabeceira, echo=? formato de saída da táboa?
<<label=sptab>>=
fitotab <- xtable(head(tabII[,1:5]), digits=0, 
                  caption="Táboa II Fitoplancto da Ria de Vigo (1951-52). 
                  Número de células por 1000 células de dinoflagelada. 
                  Cifras aproximadas calculadas sobre dos totais mensuais",
                  label="Table:abun")
print(fitotab, include.rownames=FALSE, floating=FALSE)
@

% Fai que non se visibilice nada deste bloque no documento xerados
<<label=hills>>=
...
Dos $\Sexpr{ncol(fito)}$ feitos entre o 1951 e 1952 na ría de Vigo, Margalef e compañeiros observaron entre $\Sexpr{min(div51)}$ e $\Sexpr{max(div51)}$ especies de fitoplancto despois de recontar unha media de $\Sexpr{sum(fito)}$ de \https://gl.wikipedia.org/wiki/Diatomeas diatomeas} e \href{https://gl.wikipedia.org/wiki/Dinoflaxelados}{dinoflaxelados}.
...
O mostreo na Ría no que se apreza unha maior dominancia de especies, produciuse no $\Sexpr{colnames(fito[simpson==max(simpson)])}$ mes do 1952.
 
 % Comproba a cabeceira
<<label=hills_summary>>=
fdiv <- data.frame(Species=div51,Simpson=simpson,Shannon=shannon)
rownames(fdiv) <- colnames(fito)

print(xtable(fdiv, digits=2, 
...
% Comproba a cabeceira, define que se inclúa a imaxe e define o seu tamaño da imaxe
<<label=sac>>=
